...
async def index_file(filepath):
    ...
    try:
        ...
        set_bot_state(bot_name=BOT_NAME, state_key=f"{INDEXER_FILE_PREFIX}{file_id}", state_value=file_data)
        
        logger.info(f"🤖 {BOT_NAME}: Indexado {filepath}")
    except Exception as e:
        logger.error(f"🤖 {BOT_NAME}: Error al indexar {filepath}: {e}")
        log_system_event(level="ERROR", source=BOT_NAME, message=f"Error al indexar {filepath}", details=str(e))
...
async def scan_and_index_path(path_to_scan):
    ...
    log_bot_action(bot_name=BOT_NAME, action_type="start_indexing", status="started", target=path_to_scan)
    indexed_count = 0
    for dirpath, _, filenames in os.walk(path_to_scan):
        for filename in filenames:
            ...
            indexed_count += 1
    logger.info(f"🤖 {BOT_NAME}: Escaneo e indexación completados. {indexed_count} archivos indexados.")
    log_bot_action(bot_name=BOT_NAME, action_type="start_indexing", status="completed", target=path_to_scan, result=json.dumps({"indexed_count": indexed_count}))
        ...
        try:
            ...
            if command:
                ...
                if command.command_type == 'start_indexing':
                    ...
                    send_result(bot_name=BOT_NAME, result_type="indexed_count_result", payload={
