...
class BotAuditorFilesUI:
    def __init__(self, ui_connector, folder=".", months=6):
    ...
    def audit_loop(self):
        while self.running:
            try:
                ...
                sleep_time = mode_manager.get_monitoring_interval() * 4  # Menos frecuente
                time.sleep(sleep_time)

            except Exception as e:
                self.ui_connector.send_message(f"Error en auditor de archivos: {str(e)}")
                time.sleep(120)

    def perform_comprehensive_audit(self):
        """Realizar auditoría completa de archivos"""
        try:
            self.ui_connector.send_message("🔍 Iniciando auditoría completa de archivos...")
    ...
    def _analyze_file_health(self) -> Dict[str, Any]:
        ...
        for root, dirs, files in os.walk(self.base_folder):
            ...
            for file in files:
                ...
                try:
                    ...
                    days_since_access = (datetime.now() - datetime.fromtimestamp(stat.st_atime)).days
        ...
        for files_list in file_sizes.values():
            if len(files_list) > 1:
                results['duplicate_files'][str(files_list[0])].extend(str(f) for f in files_list[1:])
    ...
    def _assess_risks(self, audit_results: Dict[str, Any]):
        ...
        # Riesgo medio
        large_old_files = [f for f in audit_results['large_files']
                          if f['days_since_access'] > 90]
    ...
    def _generate_audit_report(self, audit_results: Dict[str, Any]):
        ...
        try:
            ...
            self.ui_connector.send_message(f"📊 Auditoría: {audit_results['total_analyzed']} archivos, {total_size_gb:.2f} GB")

            # Reportar problemas encontrados
            if audit_results['old_files']:
                self.ui_connector.send_message(f"🕰️ {len(audit_results['old_files'])} archivos antiguos (> {self.months_threshold} meses)")
            ...
            if audit_results['large_files']:
                ...
                self.ui_connector.send_message(f"📁 {len(audit_results['large_files'])} archivos grandes ({large_size_gb:.2f} GB)")

            if audit_results['empty_files']:
                self.ui_connector.send_message(f"📄 {len(audit_results['empty_files'])} archivos vacíos")

            if audit_results['duplicate_files']:
                total_duplicates = sum(len(files) for files in audit_results['duplicate_files'].values())
                self.ui_connector.send_message(f"🔄 {total_duplicates} archivos duplicados")

            if audit_results['corrupt_files']:
                self.ui_connector.send_message(f"⚠️ {len(audit_results['corrupt_files'])} archivos corruptos")

            # Reportar archivos temporales/cache
            temp_cache_count = len(audit_results['temp_files']) + len(audit_results['cache_files'])
            if temp_cache_count > 0:
                self.ui_connector.send_message(f"🗑️ {temp_cache_count} archivos temporales/cache")
    ...
    def _aggressive_cleanup(self):
        ...
        try:
            ...
            if cleaned_count > 0:
                self.ui_connector.send_message(f"🧹 Limpieza agresiva: {cleaned_count} archivos eliminados")
    ...
    def _moderate_cleanup(self):
        ...
        try:
            ...
            if cleaned_count > 0:
                self.ui_connector.send_message(f"🗑️ Limpieza moderada: {cleaned_count} archivos movidos a papelera")
    ...
    def _conservative_cleanup(self):
        ...
        if total_risky > 0:
            self.ui_connector.send_message(f"ℹ️ {total_risky} archivos candidatos para limpieza (modo conservador)")
