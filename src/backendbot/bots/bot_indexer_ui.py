...
class BotIndexerUI:
    ...
    def _update_index(self):
        ...
        try:
                ...
                for file in files:
                    ...
                    try:
                        ...
                        entry = {
                            'name': file,
                            'path': str(file_path),
                            'relative_path': str(file_path.relative_to(self.base_folder)),
                            'size': stat.st_size,
            ...
            self.ui_connector.send_message(f"✅ Índice actualizado: {total_files} archivos, {size_mb:.1f} MB")
            ...
            if ext in ['.py', '.js', '.ts', '.java', '.cpp', '.c', '.cs', '.php', '.rb', '.go']:
                ...
                try:
                    with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                        ...
                        entry['code_lines'] = len([l for l in lines if l.strip() and not l.strip().startswith('#')])
                except:
                    entry['lines'] = 0
    ...
    def _quick_check(self):
        ...
        try:
            ...
            total_files = sum(1 for _, _, files in os.walk(self.base_folder)
    ...
    def _editor_mode_tasks(self):
        ...
        if recent_code:
            self.ui_connector.send_message(f"💻 {len(recent_code)} archivos de código modificados recientemente")

    def _development_mode_tasks(self):
        """Tareas específicas para modo desarrollo"""
        # Buscar archivos de configuración y proyecto
        config_files = self._find_files_by_pattern(['package.json', 'requirements.txt', 'setup.py', 'Dockerfile'])
        if config_files:
            self.ui_connector.send_message(f"🔧 {len(config_files)} archivos de configuración encontrados")
    ...
    def _find_recent_files_by_type(self, file_type: str, hours: int = 24) -> List[Dict[str, Any]]:
        """Encontrar archivos recientes por tipo"""
        cutoff_time = time.time() - (hours * 3600)
        recent_files = []

        for entry in self.file_index.values():
            if (entry.get('type') == file_type and
                entry.get('modified', 0) > cutoff_time):
