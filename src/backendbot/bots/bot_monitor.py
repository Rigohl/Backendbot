...
async def monitor_worker():
    ...
    while True:
        try:
            ...
            # Log local
            logger.info(f"🤖 Bot Monitor: Estadísticas: CPU {cpu_usage}%, RAM {ram.percent}%, Disco {disk.percent}%")
            if gpu_monitoring and "gpus" in stats:
                ...
                if gpu_info and gpu_info["memory_total_mb"] > 0:
                    vram_percent = (gpu_info["memory_used_mb"] / gpu_info["memory_total_mb"]) * 100
                    logger.info(f"🤖 Bot Monitor: GPU {gpu_info['name'][:20]}... VRAM {vram_percent:.1f}%")
