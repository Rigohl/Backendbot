...
class IntegratedDashboard(QMainWindow):
    ...
    def _setup_bots_tab(self, tabs):
        """Crear tab de bots"""
        bots_tab = QWidget()
        bots_layout = QVBoxLayout(bots_tab)

        # Lista de bots
        bots_group = QGroupBox("🤖 Bots Disponibles")
        bots_layout_inner = QVBoxLayout(bots_group)

        self.bots_list = QListWidget()
        self.bots_list.addItem("📊 Monitor - Monitorea recursos del sistema")
        self.bots_list.addItem("📁 Organizer - Organiza archivos y carpetas")
        self.bots_list.addItem("🔍 Indexer - Indexa y busca archivos")
        self.bots_list.addItem("🛡️ Guardian - Supervisa y protege")
        self.bots_list.addItem("📂 Auditor Archivos - Audita archivos antiguos")
        self.bots_list.addItem("💻 Auditor Programas - Audita programas no usados")
        bots_layout_inner.addWidget(self.bots_list)

        # Botones de acción
        buttons_layout = QHBoxLayout()
        status_button = QPushButton("📊 Ver Estado")
        status_button.clicked.connect(self._show_bot_status)
        buttons_layout.addWidget(status_button)

        execute_button = QPushButton("▶️ Ejecutar Comando")
        execute_button.clicked.connect(self._execute_bot_command)
        buttons_layout.addWidget(execute_button)

        bots_layout_inner.addLayout(buttons_layout)
        bots_layout.addWidget(bots_group)
        ...
        self.bot_results = QTextEdit()
        self.bot_results.setReadOnly(True)
        self.bot_results.setMaximumHeight(200)
        results_layout.addWidget(self.bot_results)
        bots_layout.addWidget(results_group)

        tabs.addTab(bots_tab, "🤖 Bots")

    def _setup_chat_tab(self, tabs):
        """Crear tab de chat"""
        chat_tab = QWidget()
    ...
    def _setup_tray_icon(self):
        ...
        show_action = tray_menu.addAction("Mostrar BackendBot")
        show_action.triggered.connect(self.show)

        hide_action = tray_menu.addAction("Ocultar BackendBot")
        hide_action.triggered.connect(self.hide)

        tray_menu.addSeparator()

        chat_action = tray_menu.addAction("💬 Abrir Chat")
        chat_action.triggered.connect(self._show_chat_panel)
    ...
    def _setup_timers(self):
        ...
        toggle_tray_action = QAction("Mostrar/Ocultar Bandeja", self)
    ...
    def _setup_status_bar(self):
        ...
        # Estado de bots
        self.bots_status = QLabel("🤖 Bots: OK")
        self.status_bar.addWidget(self.bots_status)
    ...
    def _setup_timers(self):
        """Configurar timers para actualizaciones automáticas"""
