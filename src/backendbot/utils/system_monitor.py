...
class SystemMonitor:
    ...
    def check_system_health(self, system_data: dict, gpu_data: dict = None) -> list:
        ...
        if th.get('cpu_high') and cpu > th.get('cpu_high'):
            alerts.append({'type': 'cpu', 'level': 'high', 'message': f'CPU usage {cpu} > {th.get("cpu_high")}'})

        if th.get('memory_high') and mem > th.get('memory_high'):
            alerts.append({'type': 'memory', 'level': 'high', 'message': f'memoria {mem} > {th.get("memory_high")}'})

        # GPU checks (simple)
        if gpu_data and isinstance(gpu_data, dict):
            for k, v in gpu_data.items():
                if isinstance(v, dict) and 'memory_total' in v:
                    if th.get('gpu_memory_high') and v.get('memory_total', 0) > th.get('gpu_memory_high'):
                        alerts.append({'type': 'gpu', 'level': 'high', 'message': f'GPU {k} memory high'})
