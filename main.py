...
class CommandRequest(BaseModel):
    """Modelo para solicitud de comando del sistema."""
    command: str
    timeout: Optional[int] = 30
    shell: Optional[bool] = False

class CommandResponse(BaseModel):
    """Modelo para respuesta de comando del sistema."""
    command: str
    output: str
    error: str
    return_code: int
...
class BackupResponse(BaseModel):
    """Modelo para respuesta de backup."""
    backup_path: str
    size_mb: float
    timestamp: str
    files_count: int
...
class PerformanceReport(BaseModel):
    """Modelo para reporte de rendimiento."""
    period_hours: int
...
class NotificationConfig(BaseModel):
    ...
    alert_types: List[str] = ["cpu", "memory", "disk"]
...
class HardwareInfo(BaseModel):
    """Modelo para información de hardware."""
    cpu_model: str
    cpu_cores: int
    cpu_threads: int
    ram_total_gb: float
...
class SystemExtendedInfo(BaseModel):
    """Modelo para información extendida del sistema."""
    basic_info: SystemInfo
...
# Configuración de comandos seguros
SAFE_COMMANDS = [
    'ping', 'tracert', 'ipconfig', 'netstat', 'tasklist', 'systeminfo',
    'whoami', 'hostname', 'ver', 'date', 'time', 'dir', 'tree'
]
