...
class IntegratedDashboard(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.logger.info(
            "BackendBot integrado inicializado correctamente", "IntegratedDashboard"
        )
    ...
    def _create_dashboard_panel(self, parent):
        ...
        # Modos del sistema
        self._create_modes_section()

        # Energía y rendimiento
        self._create_energy_section()
    ...
    def _create_advanced_systems_tab(self, tabs):
        ...
        if POWER_AVAILABLE:
            ...
            power_combo.addItems(
                ["Alto Rendimiento", "Equilibrado", "Ahorro de Energía", "Ultra Bajo"]
            )
    ...
    def _create_modes_section(self):
        """Crear sección de modos del sistema."""
        modes_group = QGroupBox("🎭 Modos del Sistema")
        modes_layout = QVBoxLayout(modes_group)

        # Modo actual
        current_mode_layout = QHBoxLayout()
        current_mode_layout.addWidget(QLabel("Modo actual:"))
        self.current_mode_label = QLabel("🟢 Normal")
        self.current_mode_label.setStyleSheet("font-weight: bold; color: green;")
        current_mode_layout.addWidget(self.current_mode_label)
        current_mode_layout.addStretch()
        modes_layout.addLayout(current_mode_layout)

        # Botones de modos
        modes_buttons_layout = QHBoxLayout()

        normal_btn = QPushButton("🟢 Normal")
        normal_btn.clicked.connect(lambda: self._change_mode("normal"))
        modes_buttons_layout.addWidget(normal_btn)

        performance_btn = QPushButton("🔴 Rendimiento")
        performance_btn.clicked.connect(lambda: self._change_mode("performance"))
        modes_buttons_layout.addWidget(performance_btn)

        economy_btn = QPushButton("🟡 Economía")
        economy_btn.clicked.connect(lambda: self._change_mode("economy"))
        modes_buttons_layout.addWidget(economy_btn)

        modes_layout.addLayout(modes_buttons_layout)

        self.dashboard_layout.addWidget(modes_group)

    def _create_energy_section(self):
        """Crear sección de gestión de energía."""
        energy_group = QGroupBox("⚡ Gestión de Energía")
        energy_layout = QVBoxLayout(energy_group)

        # Estado actual
        energy_status_layout = QHBoxLayout()
        energy_status_layout.addWidget(QLabel("Perfil actual:"))
        self.energy_profile_label = QLabel("Equilibrado")
        energy_status_layout.addWidget(self.energy_profile_label)
        energy_status_layout.addStretch()
        energy_layout.addLayout(energy_status_layout)

        # RAM actual
        ram_layout = QHBoxLayout()
        ram_layout.addWidget(QLabel("RAM objetivo:"))
        self.ram_target_label = QLabel("< 40%")
        ram_layout.addWidget(self.ram_target_label)
        ram_layout.addStretch()
        energy_layout.addLayout(ram_layout)

        # Botones de perfiles de energía
        energy_buttons_layout = QHBoxLayout()

        balanced_btn = QPushButton("⚖️ Equilibrado")
        balanced_btn.clicked.connect(lambda: self._change_energy_profile("balanced"))
        energy_buttons_layout.addWidget(balanced_btn)

        performance_btn = QPushButton("🚀 Alto Rendimiento")
        performance_btn.clicked.connect(lambda: self._change_energy_profile("high_performance"))
        energy_buttons_layout.addWidget(performance_btn)

        saver_btn = QPushButton("💚 Ahorro Energía")
        saver_btn.clicked.connect(lambda: self._change_energy_profile("power_saver"))
        energy_buttons_layout.addWidget(saver_btn)

        energy_layout.addLayout(energy_buttons_layout)

        self.dashboard_layout.addWidget(energy_group)
    ...
    def _update_bots_status(self):
        ...
        # Actualizar barra de estado
        active_bots = len(
            [b for b in self.bot_manager.bots.values() if b.is_available()]
        )
    ...
    def _show_bot_status(self):
        ...
        try:
            result = self.bot_manager._process_bot_command(
                internal_name, f"{internal_name} status"
            )
            self.bot_results.append(f"📊 Estado de {bot_name}:\n{result}\n")
        except Exception as e:
            self.bot_results.append(
                f"❌ Error obteniendo estado de {bot_name}: {str(e)}\n"
            )
    ...
    def _execute_bot_command(self):
        ...
        try:
            result = self.bot_manager._process_bot_command(
                internal_name, f"{internal_name} scan"
            )
        ...
        if self.notifications:
            try:
                ...
                QMessageBox.information(
                    self, "Éxito", "Notificación enviada correctamente"
                )
            except Exception as e:
                QMessageBox.warning(
                    self, "Error", f"Error enviando notificación: {str(e)}"
                )
    ...
    def _change_power_profile(self, profile):
        ...
        if self.power_manager:
            try:
                ...
                profile_mapping = {
                    "Alto Rendimiento": PowerProfile.HIGH_PERFORMANCE,
                    "Equilibrado": PowerProfile.BALANCED,
                    "Ahorro de Energía": PowerProfile.POWER_SAVER,
                    "Ultra Bajo": PowerProfile.ULTRA_LOW,
                }
                internal_profile = profile_mapping.get(profile, PowerProfile.BALANCED)
                success = self.power_manager.apply_profile(internal_profile)
                if success:
                    QMessageBox.information(
                        self, "Éxito", f"Perfil de energía cambiado a: {profile}"
                    )
                else:
                    QMessageBox.warning(self, "Error", "No se pudo cambiar el perfil de energía")
            except Exception as e:
    ...
    def _setup_timers(self):
        ...
        # Timer para actualizar métricas (cada 5 segundos)
        from PyQt5.QtCore import QTimer
        self.metrics_timer = QTimer()
        self.metrics_timer.timeout.connect(self._update_all_metrics)
        self.metrics_timer.start(5000)  # 5 segundos

        # Timer para actualizar estado de bots (cada 30 segundos)
        self.bots_timer = QTimer()
        self.bots_timer.timeout.connect(self._update_bots_status)
        self.bots_timer.start(30000)  # 30 segundos
    ...
    def _setup_menu(self):
        ...
        toggle_tray_action = QAction('Alternar Bandeja', self)
        toggle_tray_action.triggered.connect(self._toggle_tray)
        view_menu.addAction(toggle_tray_action)

        show_chat_action = QAction('Mostrar Chat', self)
        show_chat_action.triggered.connect(self._show_chat_panel)
    ...
    def _setup_status_bar(self):
        ...
        self.statusBar().addWidget(self.memory_status)

        self.bots_status = QLabel("🤖 Bots: --")
        self.statusBar().addWidget(self.bots_status)

        self.cpu_status = QLabel("⚡ CPU: --")
        self.statusBar().addWidget(self.cpu_status)

    def _setup_tray(self):
        """Configurar ícono de bandeja del sistema."""
        from PyQt5.QtWidgets import QMenu, QAction

        self.tray_icon = QSystemTrayIcon(self._create_icon(), self)
        self.tray_icon.setToolTip("🐝 BackendBot - Sistema Integrado")
    ...
    def _change_mode(self, mode: str):
        ...
        self.current_mode_label.setText(mode_names.get(mode, "🟢 Normal"))
        self.current_mode_label.setStyleSheet(f"font-weight: bold; color: {self._get_mode_color(mode)};")

        # Aplicar configuración según modo
        if mode == "performance":
            # Modo rendimiento: optimizar para velocidad
            if self.power_manager:
                self.power_manager.apply_profile("high_performance")
            self.ram_target_label.setText("< 60%")
        elif mode == "economy":
            # Modo economía: optimizar para ahorro
            if self.power_manager:
                self.power_manager.apply_profile("power_saver")
            self.ram_target_label.setText("< 30%")
        else:
            # Modo normal
            if self.power_manager:
                self.power_manager.apply_profile("balanced")
            self.ram_target_label.setText("< 40%")

        QMessageBox.information(self, "Modo cambiado", f"Modo del sistema cambiado a: {mode_names.get(mode, 'Normal')}")

    def _get_mode_color(self, mode: str) -> str:
        """Obtener color para el modo."""
        colors = {
            "normal": "green",
            "performance": "red",
            "economy": "orange"
    ...
    def _change_energy_profile(self, profile: str):
        """Cambiar perfil de energía."""
        profile_names = {
        ...
        self.energy_profile_label.setText(profile_names.get(profile, "Equilibrado"))
        ...
        if self.power_manager:
            ...
            profile_mapping = {
                ...
                "high_performance": PowerProfile.HIGH_PERFORMANCE,
                "power_saver": PowerProfile.POWER_SAVER
            }
            internal_profile = profile_mapping.get(profile, PowerProfile.BALANCED)
            success = self.power_manager.apply_profile(internal_profile)
            if success:
                QMessageBox.information(self, "Perfil cambiado", f"Perfil de energía cambiado a: {profile_names.get(profile, 'Equilibrado')}")
            else:
                QMessageBox.warning(self, "Error", "No se pudo cambiar el perfil de energía")
