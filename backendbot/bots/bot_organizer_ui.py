...
class BotOrganizerUI:
    ...
    def organizer_loop(self):
        while self.running:
            try:
                ...
                if (
                    mode_manager.should_cleanup()
                    and (current_time - self.last_scan) > self.scan_interval
                ):
                ...
                sleep_time = (
                    mode_manager.get_monitoring_interval() * 3
                )  # Menos frecuente
    ...
    def perform_full_scan(self):
        ...
        try:
            self.ui_connector.send_message(
                "🔍 Iniciando escaneo completo de archivos..."
            )
    ...
    def _report_statistics(self, stats: dict[str, Any]):
        ...
        try:
            ...
            self.ui_connector.send_message(
                f"📊 Estadísticas: {stats['total_files']} archivos, {size_gb:.2f} GB"
            )
            ...
            self.ui_connector.send_message(
                f"📏 Tamaños: P:{size_dist['small']} M:{size_dist['medium']} G:{size_dist['large']} XG:{size_dist['huge']}"
            )

            # Distribución por edad
            age_dist = stats["by_age"]
            self.ui_connector.send_message(
                f"🕐 Edad: Hoy:{age_dist['recent']} Sem:{age_dist['week']} Mes:{age_dist['month']} Ant:{age_dist['old']}"
            )

            # Alertas
            if stats["empty_files"]:
                self.ui_connector.send_message(
                    f"⚠️ {len(stats['empty_files'])} archivos vacíos encontrados"
                )

            if stats["duplicates"]:
                self.ui_connector.send_message(
                    f"⚠️ {len(stats['duplicates'])} archivos duplicados detectados"
                )
    ...
    def _basic_organization(self):
        ...
        try:
            ...
            if organized_count > 0:
                self.ui_connector.send_message(
                    f"📂 Organizados {organized_count} archivos por tipo"
                )
