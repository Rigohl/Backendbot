...
class BotOptimizerUI:
    ...
    def optimizer_loop(self):
        while self.running:
            try:
                ...
                sleep_time = (
                    mode_manager.get_monitoring_interval() * 2
                )  # Menos frecuente que el monitoreo
        ...
        try:
            ...
            if top_procs:
                ...
                for pid, name, cpu, mem in top_procs:
                    ...
                    proc_list.append(
                        f"{name[:15]}{focus_indicator} (CPU:{cpu:.1f}%, RAM:{mem:.1f}%)"
                    )
    ...
    def perform_optimization(self):
        ...
        try:
            ...
            self.ui_connector.send_message(
                f"✅ Optimización completada para modo {mode_name.capitalize()}"
            )
    ...
    def optimize_for_gaming(self):
        ...
        if closed_count > 0:
            self.ui_connector.send_message(
                f"🎮 Cerrados {closed_count} procesos no esenciales para gaming"
            )
    ...
    def optimize_for_streaming(self):
        ...
        self.ui_connector.send_message(
            "🎥 Modo streaming: Optimizando para transmisión estable"
        )
    ...
    def optimize_for_editing(self):
        ...
        if prioritized > 0:
            self.ui_connector.send_message(
                f"💻 Priorizados {prioritized} procesos de desarrollo"
            )
    ...
    def optimize_for_development(self):
        ...
        self.optimize_for_editing()
        self.ui_connector.send_message(
            "🔧 Modo desarrollo: Optimización intensiva activada"
        )

    def optimize_for_relax(self):
        """Optimizaciones mínimas para modo relax."""
        self.ui_connector.send_message(
            "😌 Modo relax: Optimización mínima para navegación"
        )
