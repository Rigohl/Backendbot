...
async def calculate_file_hash(filepath, hash_algo=hashlib.md5, chunk_size=4096):
    ...
    try:
        ...
        log_system_event(
            level="ERROR",
            source=BOT_NAME,
            message=f"Error al calcular hash de {filepath}",
            details=str(e),
        )
        return None
        ...
        for filename in filenames:
            ...
            if file_hash in hashes:
                ...
                if not found_in_duplicates:
                    ...
                    duplicates.append(
                        {"hash": file_hash, "paths": [hashes[file_hash], filepath]}
                    )
            else:
                hashes[file_hash] = filepath
    logger.info(
        f"🤖 {BOT_NAME}: Escaneo completado. Encontrados {len(duplicates)} grupos de duplicados."
    )
    log_bot_action(
        bot_name=BOT_NAME,
        action_type="scan_duplicates",
        status="completed",
        target=path_to_scan,
        result=json.dumps({"count": len(duplicates)}),
    )
    return duplicates
        ...
        try:
            ...
            if command:
                ...
                if command.command_type == "scan_duplicates":
                    ...
                    set_bot_state(
                        bot_name=BOT_NAME,
                        state_key=ORGANIZER_DUPLICATES_KEY,
                        state_value=duplicates,
                    )
                    send_result(
                        bot_name=BOT_NAME,
                        result_type="scan_duplicates_result",
                        payload={
                    ...
                    log_bot_action(
                        ...
                        status="started",
                        target=json.dumps(files_to_delete),
                    )
                    for filepath in files_to_delete:
                        try:
                            send2trash.send2trash(
                                filepath
                            )  # Usar send2trash en lugar de os.remove
                            deleted_count += 1
                            logger.info(
                                f"🤖 {BOT_NAME}: Enviado a papelera: {filepath}"
                            )
                        except Exception as e:
                            logger.error(
                                f"🤖 {BOT_NAME}: Error al enviar a papelera {filepath}: {e}"
                            )
                            log_system_event(
                                level="ERROR",
                                source=BOT_NAME,
                                message=f"Error al enviar a papelera {filepath}",
                                details=str(e),
                            )
                    send_result(
                        bot_name=BOT_NAME,
                        result_type="delete_duplicates_result",
                        payload={
                            "command_id": command.id,
...
class BotOrganizer:
    ...
    def organize_files(self, source_path: str, dest_path: str, rules: dict) -> dict:
        ...
        return {"total_files": total_files, "organized_files": organized_files}
