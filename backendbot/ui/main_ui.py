...
from src.backendbot.utils.advanced_command_processor import \
    advanced_command_processor
...
class BackendBotUI:
        ...
        try:
            ...
            if recommendations:
                ...
                self.panel.show_message(
                    "📚 No hay recomendaciones disponibles aún. El sistema está aprendiendo..."
                )
        except Exception as e:
    ...
    def show_learning_stats(self):
        ...
        try:
            ...
            self.panel.show_message(
                f"• Acciones aprendidas: {stats['total_actions_learned']}"
            )
            self.panel.show_message(
                f"• Métricas registradas: {stats['total_metrics_recorded']}"
            )
            self.panel.show_message(
                f"• Patrones analizados: {stats['patterns_analyzed']}"
            )
            self.panel.show_message(
                f"• Sistema activo: {'Sí' if stats['learning_active'] else 'No'}"
            )
        except Exception as e:
    ...
    def show_task_stats(self):
        ...
        try:
            ...
            self.panel.show_message(
                f"• Tareas deshabilitadas: {stats['disabled_tasks']}"
            )
            ...
            self.panel.show_message(
                f"• Programador activo: {'Sí' if stats['running'] else 'No'}"
            )
        except Exception as e:
            self.panel.show_message(
                f"Error obteniendo estadísticas de tareas: {str(e)}"
            )
    ...
    def show_task_list(self):
        ...
        try:
            ...
            for task in tasks:
                ...
                self.panel.show_message(
                    f"{status} {task['name']} ({freq}) - {priority}"
                )
    ...
    def run_tasks_manually(self):
        ...
        try:
            ...
            self.panel.show_message(
                "✅ Comando recibido. Las tareas se ejecutarán según su programación."
            )
            self.panel.show_message(
                "💡 Usa 'tareas lista' para ver todas las tareas disponibles"
            )
        except Exception as e:
    ...
    def _handle_task_management_from_nlp(
        self, recognized_command: str, original_cmd: str
    ):
    ...
    def _show_detailed_system_status(self):
        ...
        try:
            ...
            self.panel.show_message(
                f"• Memoria: {memory.percent}% ({memory.used / 1024 / 1024 / 1024:.1f} GB usado)"
            )
            self.panel.show_message(
                f"• Disco: {disk.percent}% ({disk.free / 1024 / 1024 / 1024:.1f} GB libre)"
            )
            self.panel.show_message(
                f"• Modo actual: {mode_manager.get_mode_info()['mode'].capitalize()}"
            )
        except Exception as e:
    ...
    def _show_detailed_performance(self):
        ...
        try:
            ...
            self.panel.show_message(
                f"• CPU Frecuencia: {cpu_freq.current:.0f} MHz"
                if cpu_freq
                else "• CPU Frecuencia: N/A"
            )
            self.panel.show_message(
                f"• Memoria Total: {memory.total / 1024 / 1024 / 1024:.1f} GB"
            )
            self.panel.show_message(
                f"• Memoria Disponible: {memory.available / 1024 / 1024 / 1024:.1f} GB"
            )
            self.panel.show_message(
                f"• Red Enviado: {network.bytes_sent / 1024 / 1024:.1f} MB"
            )
            self.panel.show_message(
                f"• Red Recibido: {network.bytes_recv / 1024 / 1024:.1f} MB"
            )
        except Exception as e:
            self.panel.show_message(
                f"Error obteniendo métricas de rendimiento: {str(e)}"
            )
