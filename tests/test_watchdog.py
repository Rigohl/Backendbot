...
from backendbot.watchdog import _handle_process_monitoring, watchdog
...
@pytest.mark.asyncio
async def test_handle_process_monitoring_high_usage_notify(
    mock_store_decision, mock_notify, mock_log_event
):
    ...
    mock_process.suspend.side_effect = (
    ...
    with patch("time.time", return_value=100 + Settings().CHECK_TIME + 1):
        ...
        mock_notify.assert_called_once_with(
            "⚠️ test_process alto consumo. Revisa dashboard.", subtle=True
        )
        mock_store_decision.assert_called_once_with(
            "test_process", "notificado", 90.0, 5000.0
        )
...
@patch("backendbot.watchdog.store_watchdog_decision")
@pytest.mark.asyncio
async def test_handle_process_monitoring_suspend(
    mock_store_decision, mock_notify, mock_log_event
):
    mock_process = MagicMock()
    mock_process.info = {
    ...
    with patch("time.time", return_value=100 + Settings().CHECK_TIME + 1):
        ...
        mock_store_decision.assert_called_once_with(
            "test_process", "suspendido", 90.0, 5000.0
        )
...
@patch("backendbot.watchdog.store_watchdog_decision")
@pytest.mark.asyncio
async def test_handle_process_monitoring_ignored(
    mock_store_decision, mock_notify, mock_log_event
):
    mock_process = MagicMock()
    mock_process.info = {
    ...
    with patch("time.time", return_value=100 + Settings().CHECK_TIME + 1):
        ...
        mock_store_decision.assert_called_once_with(
            "test_process", "ignorado", 90.0, 5000.0
        )
        assert 123 not in uso_alto
...
@pytest.mark.asyncio
async def test_watchdog_main_loop(
    mock_process_iter,
    mock_load_memory,
    mock_store_decision,
    mock_notify,
...
@pytest.mark.asyncio
async def test_watchdog_main_loop_error_handling(
    mock_process_iter,
    mock_load_memory,
    mock_store_decision,
    mock_notify,
    ...
    mock_log_event.assert_called_once_with(
        "Error en watchdog principal: ProcessIterError"
    )
